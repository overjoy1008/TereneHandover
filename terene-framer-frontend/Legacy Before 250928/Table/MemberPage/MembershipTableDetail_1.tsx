...
export function MembershipTableDetail() {
    ...
    //     const startMonth = signupDate.getMonth() + 1
    //     const startDay = signupDate.getDate()
    ...
    //     return `${startYear}.${pad(startMonth)}.${pad(startDay)} ~ ${endYear}.${pad(endMonth)}.${pad(endDay)}`
    // }
    ...
    const formatUsagePeriod = (signupDateStr?: string): string => {
        ...
        // signupDate에서 월/일만 사용
        const month = signupDate.getMonth() + 1
        const day = signupDate.getDate()

        // year 상태에서 시작/종료 연도 파싱
        // 예) "2025-2026"이면 startYear=2025, endYear=2026
        // 예) "2025"처럼 단일 값이면 endYear=startYear+1
    ...
    // // 사용기한 "종료일"을 YYYY-MM-DD로 반환
    // const getUsageEndDateISO = (
    //     signupDateStr?: string,
    //     selectedYear?: string
    // ): string | null => {
    //     if (!signupDateStr || !selectedYear) return null

    //     const sd = new Date(signupDateStr)
    //     if (isNaN(sd.getTime())) return null

    //     const month0 = sd.getMonth() // 0-based
    //     const day = sd.getDate()

    //     let startYear: number, endYear: number
    //     if (selectedYear.includes("-")) {
    //         const [sy, ey] = selectedYear.split("-").map((v) => parseInt(v, 10))
    //         startYear = sy
    //         endYear = ey
    //     } else {
    ...
    //     const startDate = new Date(startYear, month0, day)
    //     const endDate = new Date(startDate)
    //     endDate.setFullYear(endDate.getFullYear() + 1)
    //     endDate.setDate(endDate.getDate() - 1)
