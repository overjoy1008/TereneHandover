...
const parseSignupMonthDay = (signupDateStr: string) => {
    const m = signupDateStr.match(/^(\d{4})[\/.-](\d{1,2})[\/.-](\d{1,2})/)
...
const calcMembershipYearRange = (signupDateStr: string, now: Date) => {
    const md = parseSignupMonthDay(signupDateStr)
    if (!md) return null

    const { month, day } = md
    const nowYear = now.getFullYear()
    const thisYearAnniversary = new Date(nowYear, month - 1, day)
    const startYear = now < thisYearAnniversary ? nowYear - 1 : nowYear
    return `${startYear}-${startYear + 1}`
}

const buildYearOptions = (signupDateStr?: string, now?: Date) => {
    if (!signupDateStr || !now) return []
    ...
    const signupYear = Number(signupDateStr.match(/^(\d{4})/)?.[1])
    if (!signupYear) return []

    const currentRange = calcMembershipYearRange(signupDateStr, now)
    if (!currentRange) return []
    const currentStartYear = Number(currentRange.split("-")[0])

    const out: string[] = []
    for (let y = signupYear; y <= currentStartYear; y++) {
        out.push(`${y}-${y + 1}`)
    }
    return out
}
...
export function MembershipTableDetail({
    ...
    const didAutoSetYearRef = React.useRef(false)
    ...
    const yearOptions = React.useMemo(() => {
        return buildYearOptions(signupDate, kstNow())
    }, [signupDate])
    ...
    React.useEffect(() => {
        ...
        if (didAutoSetYearRef.current) return

        const next = calcMembershipYearRange(signupDate, kstNow())
        if (next && next !== year) setYear(next)

        didAutoSetYearRef.current = true
    }, [signupDate])
