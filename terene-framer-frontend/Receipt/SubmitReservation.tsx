...
interface MembershipCouponByDate {
    [dateStr: string]: {
    ...
    return (props) => {
        ...
        const transformCouponDetails = (details) =>
            (details || []).map((couponDetail) => {
                const membership =
                    store.membershipCouponByDate as MembershipCouponByDate
                let targetDate: string | null = null

                if (membership) {
                    Object.entries(membership).forEach(([dateStr, info]) => {
                        if (info.coupon_instance_id === couponDetail.id) {
                            targetDate = dateStr
                        }
                    })
                }
        ...
        const handleClick = async () => {
            ...
            // alert(
            //     `special_requests: ${formStore.special_requests}\nadminBypassCode: ${adminBypassCode}`
            // )
        ...
        const handleAgree = async () => {
            ...
            try {
                ...
                const finalAmount = Number(store.finalPrice)
